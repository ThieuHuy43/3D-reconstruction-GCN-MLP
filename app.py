...
def main():
    ...
    if 'video_basename' not in st.session_state:
        st.session_state.video_basename = None
    ...
    if uploaded_file is not None:
        # Tạo output_dir dựa trên tên file
        video_basename = os.path.splitext(uploaded_file.name)[0]
        output_dir = f'./demo/output/{video_basename}/'
        
        # Display video info
        # Lưu video tạm thời để đọc thông tin
        temp_video_path = f'./demo/output/temp_{uploaded_file.name}'
        ...
        if st.button("🚀 Start Pose Estimation", type="primary"):
            process_video(temp_video_path, uploaded_file.name, fix_z)
            # Lưu trạng thái đã xử lý
            st.session_state.processed = True
            st.session_state.output_dir = output_dir
            st.session_state.video_basename = video_basename
        
        # Hiển thị kết quả nếu đã xử lý xong
        if st.session_state.processed and st.session_state.output_dir == output_dir:
            display_results(st.session_state.output_dir, st.session_state.video_basename)
...
def process_video(video_path, video_name, fix_z):
...
def display_results(output_dir, video_name):
    ...
    if os.path.exists(output_video_path):
        ...
        st.download_button(
            ...
            mime="video/mp4",
            key="download_video"  # Thêm key duy nhất
        )
...
def display_sample_frames(output_dir):
    ...
    with col1:
        if os.path.exists(pose2d_dir):
            ...
            if pose2d_files:
                frame_idx = st.slider("Select 2D Frame", 0, len(pose2d_files)-1, 0, key="slider_2d")
    ...
    with col2:
        if os.path.exists(pose3d_dir):
            ...
            if pose3d_files:
                frame_idx = st.slider("Select 3D Frame", 0, len(pose3d_files)-1, 0, key="slider_3d")
        ...
        try:
            ...
            with open(keypoints_3d_path, 'rb') as f:
                st.download_button(
                    ...
                    key="download_3d_data"  # Thêm key duy nhất
                )
